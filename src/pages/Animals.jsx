...
export default function Animals() {
  ...
  function setCategory(cat) {
    ...
    if (activeOrigin !== 'all') p.origin = activeOrigin
    setSearchParams(p)
  }
  ...
  }), [list, activeCategory, activeOrigin, search])
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        {/* Loading spinner */}
        {loading ? (
          <div className="flex items-center justify-center py-32 gap-3 text-green-700">
          ...
          <>
            ...
            <div className="mb-4 sm:mb-6">
              <div className="relative w-full sm:max-w-md">
                ...
                <input type="text" placeholder={tr(t.animals.search)} value={search}
            ...
            <div className="flex gap-2 mb-3 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:flex-wrap">
              <button onClick={() => setCategory('all')}
                className={`flex-shrink-0 px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm font-medium transition-all whitespace-nowrap ${
                  activeCategory === 'all' ? 'bg-green-700 text-white shadow-md' : 'bg-white text-gray-600 border border-gray-200 hover:border-green-400 hover:text-green-700'
                }`}>
                {tr(t.animals.all)} ({list.length})
              </button>
              {CATEGORIES.map(cat => {
                const count = list.filter(a => a.category === cat.id).length
                return (
                  <button key={cat.id} onClick={() => setCategory(cat.id)}
                    className={`flex-shrink-0 px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm font-medium transition-all flex items-center gap-1 whitespace-nowrap ${
                      activeCategory === cat.id ? 'bg-green-700 text-white shadow-md' : 'bg-white text-gray-600 border border-gray-200 hover:border-green-400 hover:text-green-700'
                    }`}>
    ...
    <Link to={`/animals/${animal.id}`}
      ...
      <div className="relative h-44 overflow-hidden bg-green-50">
        {animal.image
          ...
          : <div className="w-full h-full flex items-center justify-center text-5xl sm:text-6xl">{category?.icon}</div>
        }
      ...
      <div className="p-4">
        <h3 className="font-bold text-green-900 group-hover:text-green-600 transition-colors text-sm sm:text-base">{animal.name}</h3>
